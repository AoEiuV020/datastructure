...
template < typename T > class Polynomial:public List < int >
{
  private:
	class Term;
...
template < typename T > class Polynomial < T >::Term:public Node
{
  public:
	Term(const Term &);
...
template < typename T > Polynomial < T >::Term::Term(const Term & term)
{
	coef = term.coef;
	data = term.data;
	prev = next = nullptr;
...
template < typename T > Polynomial < T >::Term::Term(const T & c, const int &d)
{
	coef = c;
	data = d;
}
...
// */
template < typename T > Polynomial < T > &Polynomial < T >::operator+=(const Term & term)
{
	Node *node;
	for (node = head->next; (node != tail) && (node->data > term.data); node = node->next)
	{							/* nothing... */
	}
...
template < typename T > Polynomial < T > Polynomial < T >::operator*(const Term & term)
{
	...
	tmp += (*this);
	Node *node;
	for (node = tmp.head->next; node != tmp.tail; node = node->next)
	{
		node->data += term.data;
		static_cast < Term * >(node)->coef *= term.coef;
	}
...
template < typename T > Polynomial < T >::Polynomial(const T & c, const int &d):Polynomial()
{
	operator+=(Term(c, d));
}

template < typename T > Polynomial < T > &Polynomial < T >::operator+=(const Polynomial & poly)
{
	Node *node;
...
template < typename T > Polynomial < T > &Polynomial < T >::operator*=(const Polynomial & poly)
{
	...
	for (node = poly.head->next; node != poly.tail; node = node->next)
	{
		operator+=(tmp * (*static_cast < Term * >(node)));
	}
	return (*this);
}
...
template < typename T > void Polynomial < T >::input(std::istream & in)
{
	T coef;
	int data;
	in.clear();
...
template < typename T > void Polynomial < T >::output(std::ostream & out) const
{
	Node *node;
	if (empty())
	{
	...
	{
		out << std::showpos << static_cast <
			Term * >(node)->coef << std::noshowpos << "x^" << node->data;
	}
}

template < typename T > std::istream & operator>>(std::istream & in, Polynomial < T > &poly)
{
	poly.input(in);
	return in;
}

template < typename T > std::ostream & operator<<(std::ostream & out, const Polynomial < T > &poly)
{
	poly.output(out);
	return out;
}
...
int main()
{
	...
	cout << "Input Polynomial like" << endl;
